...
const MAX_SIZE_BYTES = 5 * 1024 * 1024; // 5 MB — fotos de produto pesam mais que logos
...
/**
 * Upload de foto de produto (imagem principal, galeria) para o Storage
 * público do Supabase. Rota protegida pelo proxy (não está em
 * PUBLIC_PREFIXES) — só acessível com sessão válida do painel.
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error("[uploads] erro ao enviar imagem de produto:", error);
    return NextResponse.json(
