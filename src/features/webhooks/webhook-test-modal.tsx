...
export function WebhookTestModal({ endpoint }: { endpoint: WebhookEndpointRow }) {
  const [open, setOpen] = React.useState(false);
  const [event, setEvent] = React.useState(endpoint.events[0] ?? WEBHOOK_EVENTS[0].key);
  const [state, formAction, pending] = useActionState<TestWebhookResult | null, FormData>(
    sendTestWebhookAction,
    null,
  );
        ...
        <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-4">
          <form action={formAction} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              >
                {(endpoint.events.length > 0 ? endpoint.events : WEBHOOK_EVENTS.map((e) => e.key)).map(
                  (key) => (
            ...
            <div className="space-y-3 border-t pt-4">
              {state.ok ? (
                <Alert variant="success">
                  <CheckCircle2 />
                  <AlertDescription>
                    Respondeu com sucesso em {formatNumber(state.durationMs ?? 0)}ms.
                  </AlertDescription>
                </Alert>
              ) : (
                <Alert variant="destructive">
                  <AlertCircle />
                  <AlertDescription>{state.error ?? "O destino não respondeu com sucesso."}</AlertDescription>
                </Alert>
              )}

              <div className="space-y-1">
                <p className="text-muted-foreground text-xs font-medium">Status HTTP</p>
                <p className="font-mono text-sm">{state.httpStatus ?? "—"}</p>
              </div>

              {state.requestBody && (
                <div className="space-y-1">
                  <p className="text-muted-foreground text-xs font-medium">Corpo enviado</p>
              ...
              {state.responseBody && (
                <div className="space-y-1">
                  <p className="text-muted-foreground text-xs font-medium">Resposta recebida</p>
